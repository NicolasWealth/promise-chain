    ...
    <PageFrame>
      ...
      <main>
        ...
        <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">{profileHistory.map((commitment) => <div key={commitment.reference} className="border border-rule p-5"><div className="flex items-center justify-between gap-3"><StatusBadge status={commitment.status} /><span className="font-mono text-[10px] text-faint">{commitment.reference}</span></div><h3 className="mt-6 font-bold">{commitment.title}</h3><p className="mt-2 font-mono text-sm">{commitment.amount} {commitment.token}</p></div>)}</div>
        </section>
