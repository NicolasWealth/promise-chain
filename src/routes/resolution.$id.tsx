    ...
    <Shell>
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        ...
        <div className="mx-auto max-w-3xl py-14 text-center sm:py-20">
          ...
          >
            {failed ? <CircleX className="size-8" /> : <Check className="size-8" />}
          </div>
          ...
          <h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-5xl">
            {failed ? "Commitment failed" : "Commitment completed"}
          </h1>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px] lg:items-start">
          <div className="space-y-6">
            <EvidencePanel
              ...
              verified={!failed && commitment.mergeStatus === "merged"}
            />
              ...
              <div className="mt-5 grid gap-5 sm:grid-cols-2">
                <div>
                  <p className="text-xs text-muted-foreground">
                    {failed ? "Deadline" : "Completion timestamp"}
                  </p>
                  <p className="mt-1 font-semibold">
                    {failed ? commitment.deadline : commitment.resolvedAt}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">
                    {failed ? "Missing evidence" : "Verification result"}
                  </p>
          ...
          <aside className="border border-rule bg-panel p-5 sm:p-6">
            <SectionEyebrow>Escrow outcome</SectionEyebrow>
            <p className="mt-5 font-mono text-[10px] uppercase tracking-[.14em] text-faint">
              {failed ? "Funds returned to creator" : "Funds released to beneficiary"}
            </p>
            <p className="mt-2 text-3xl font-bold">
              {commitment.amount}{" "}
              <span className="text-sm text-muted-foreground">{commitment.token}</span>
            </p>
            <div className="mt-6 border-t border-rule pt-2">
              <TransactionRow
                label={failed ? "Escrow settlement" : "Release transaction"}
                hash={commitment.releaseHash ?? commitment.transactionHash}
            ...
            <Button asChild variant="outline" className="mt-6 w-full">
              <Link to="/profile">View public reputation</Link>
            </Button>
          </aside>
        </div>
      </main>
