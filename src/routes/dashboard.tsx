...
function Dashboard() {
  ...
  return <Shell>
    <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <div className="mt-12 flex flex-wrap items-center justify-between gap-4 border-b border-rule pb-3"><div className="flex flex-wrap gap-1">{(['all', 'active', 'completed', 'failed'] as const).map((item) => <Button key={item} variant={filter === item ? 'primary' : 'ghost'} size="sm" onClick={() => setFilter(item)}>{item[0].toUpperCase() + item.slice(1)}</Button>)}</div><span className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.14em] text-faint"><SlidersHorizontal className="size-3.5" /> {filtered.length} records</span></div>
