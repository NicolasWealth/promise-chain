...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Fund measurable commitments and build a public record of delivering." },
      { property: "og:title", content: "CommitChain — Make promises worth keeping" },
      { property: "og:description", content: "Fund measurable commitments and build a public record of delivering." },
    ],
  }),
  component: Index,
});
      ...
      <main>
        <section className="mx-auto grid max-w-6xl gap-12 px-5 pb-20 pt-16 sm:px-8 sm:pt-24 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:gap-16 lg:pb-28">
          ...
          <div className="relative animate-[cc-rise_.7s_.1s_ease-out_both]">
            ...
            <div className="mt-3 flex items-center gap-2 px-1 font-mono text-[10px] uppercase tracking-[.14em] text-faint"><ShieldCheck className="size-3.5 text-lime-soft" /> Escrow-backed · publicly verifiable</div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
          <div className="flex flex-wrap items-end justify-between gap-5"><div><SectionEyebrow>Public reputation</SectionEyebrow><h2 className="mt-3 text-3xl font-bold tracking-tight">A track record you can inspect.</h2></div><Link to="/profile" className="inline-flex items-center gap-1 text-sm font-bold text-lime-soft hover:text-foreground">View Project Alpha <ArrowUpRight className="size-4" /></Link></div>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">{profileHistory.map((commitment) => <div key={commitment.reference} className="border border-rule p-5"><div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-lime-soft"><Check className="size-3.5" /> {commitment.status === 'failed' ? 'FAILED' : 'COMPLETED'}</span><span className="font-mono text-[10px] text-faint">{commitment.reference}</span></div><h3 className="mt-6 font-bold">{commitment.title}</h3><p className="mt-2 font-mono text-sm">{commitment.amount} {commitment.token}</p></div>)}</div>
        </section>

        <section className="bg-ink text-background"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-8 px-5 py-14 sm:px-8"><div><SectionEyebrow>Start with a promise</SectionEyebrow><h2 className="mt-3 max-w-xl text-3xl font-bold tracking-tight sm:text-4xl">The next milestone deserves a deadline.</h2></div><Button asChild variant="accent" size="lg"><Link to="/create">Create a commitment <ArrowUpRight className="size-4" /></Link></Button></div></section>
      </main>
      <Footer />
    </PageFrame>
  );
